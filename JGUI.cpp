...
void JGUI::Image(Vector2 start, Vector2 end, ID3D11ShaderResourceView * image, Vector2 resolution)
{
	Vector2 screen = Input.GetDesktopResolution();
	float min_x = 1 / (resolution.x / start.x), min_y = 1 / (resolution.y / start.y);
	float max_x = 1 / (resolution.x / end.x), max_y = 1 / (resolution.y / end.y);

	this->drawList->AddImage(image, ImVec2((screen.x * min_x) - 4, (screen.y * min_y)), ImVec2(screen.x * max_x, screen.y * max_y));
}

void JGUI::Image(float ndcMinX, float ndcMinY, float ndcMaxX, float ndcMaxY, ID3D11ShaderResourceView * image)
{
	Vector2 screen = Input.GetDesktopResolution();
